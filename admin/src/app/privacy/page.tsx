...
export default function AdminPrivacyPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getLegal(siteId, "privacy");
      const page = res.page;
      setPageTitle(page.title || "PRIVACY POLICY");
      setSubtitle(
        page.subtitle ||
          "HOW WE COLLECT, USE, AND PROTECT YOUR PERSONAL INFORMATION."
      );
      ...
      if (page.sections?.length) {
        ...
        setSections([{ title: "Information We Collect", body: page.content || "" }]);
      }
    } catch {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      await updateLegal(siteId, "privacy", {
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-[#6B7280] mt-0.5">
                  Manage site privacy policy document sections and metadata
                </p>
              </div>
            </div>
            <button
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                <div>
                  ...
                  <p className="text-xs text-[#6B7280] mt-1">
                    Add headings and paragraphs for your privacy policy.
                  </p>
                </div>
                <button
                  type="button"
